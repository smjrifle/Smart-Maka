...
char * sendBuf="D:W"; //format is MODE:SPEED
...
void displayMenu(boolean includeHeader=true)
{
  ...
  {
  Serial.println("******************************************************************");
  Serial.println("*                  SmartMaka preliminart work                    *");
  }
  Serial.println("******************************************************************");
...
static handleInput(char response){
   input = mySerial.read();
    ...
    else if((input == 'O' || input == 'F') && (PINdirection == 'W') && (PINmode == 'D')){
        PINdirection = input;
     Serial.print("Switched Pin Direction to [" );
      Serial.print(input == 'W' ? "Write" : input == 'R' ? "Read");
      Serial.println("]");
