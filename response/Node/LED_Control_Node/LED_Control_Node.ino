...
#define DEVICENAME    "SW"   // name of the node
...
void setup(void)
{
  ...
  pinMode(LED, OUTPUT);
  Serial.println("Listening for LED  requests...\n");
  Serial.println();
  
  if(!registered){
      sprintf(sendBuf, "%s:%c",DEVICENAME,'r');  // the Register process is in [Deice name:code] i.e [SW:r]
...
void loop()
  ...
  if (radio.ReceiveComplete())
  {
    if (radio.CRCPass())
    {
      ...
      if (*radio.DataLen == 3)  // data for Control
      pinValue =radio.Data[2];
      
      else if(*radio.DataLen == 5){ // register response with [NodeID:NetworkID:GatewayID] i.e. [2:212:1]
      if(radio.Data[0] == NODEID && radio.Data[2] == NETWORKID && radio.Data[4] == NODEID)
      registered=true;
      }
